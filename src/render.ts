...
const mapHTMLAttributes = <T>(attributes: HTMLAttributes, htmlElememnt: SVGSVGElement | SVGRectElement) => Object.entries(attributes).forEach(([key, value]) => htmlElememnt.setAttribute(key, value))
...
const renderUnit = (svgParent: SVGSVGElement, unit: IConfig['unit'], x: ILocation['x'], y: ILocation['y']) => (fill: string): SVGRectElement => svgParent.appendChild(createRect({x: `${x * unit}`, y: `${y * unit}`, width: `${unit}`, height: `${unit}`, fill}))

export const render = (store: Store, svgParent: SVGSVGElement, unit: number) => {
  store.getMap().forEach((row, y) => row.forEach((field, x) => {
